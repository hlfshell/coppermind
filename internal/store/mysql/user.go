...
func (store *MySQLStore) CreateUser(user *users.User, password string) error {
...
func (store *MySQLStore) GetUser(id string) (*users.User, error) {
...
func (store *MySQLStore) GetUserAuth(id string) (*internal_users.UserAuth, error) {
	query := `SELECT {0} FROM {1} WHERE id = ?`
...
func (store *MySQLStore) SaveUserAuth(auth *internal_users.UserAuth) error {
	query := `UPDATE {0}
		SET
			password = ?,
			reset_token = ?,
...
func (store *MySQLStore) GenerateUserPasswordResetToken(id string) (string, error) {
...
func (store *MySQLStore) ResetPassword(id string, token string, password string) error {
	auth, err := store.GetUserAuth(id)
	if err != nil {
		return err
	}
...
func (store *MySQLStore) DeleteUser(id string) error {
...
func (store *MySQLStore) sqlToUsers(rows *sql.Rows) ([]*users.User, error) {
	defer rows.Close()
...
func (store *MySQLStore) sqlToUserAuths(rows *sql.Rows) ([]*internal_users.UserAuth, error) {
	defer rows.Close()
